...
export function BlogForm({ post }: { post?: BlogPost | null }) {
  ...
  const imageUrlDefault =
    post?.imageUrl && !isLocalUploadPath(post.imageUrl) ? post.imageUrl : "";
  return (
    <form action={saveBlogPost} className="mx-auto max-w-3xl space-y-4">
      ...
      <div>
        ...
        <input
          ...
          placeholder="Masalan: hijoma-haqida yoki bo‘sh"
      ...
      <div className="space-y-2 rounded-lg border border-slate-200 bg-slate-50/80 p-4">
        ...
        {post?.imageUrl ? (
          <div className="relative h-28 w-full max-w-[14rem] overflow-hidden rounded-lg border border-slate-200 shadow-sm">
            <Image
              src={post.imageUrl}
              alt=""
              fill
